...
vax_sub2 %>% filter(GEOID == "78725")
vax_sub2 %>% filter(GEOID == "78660")
...
vax_o <- vax_sub2_my %>%
  mutate(coverage_one_dose_total_pop = NA) %>% 
  select(ZCTA = GEOID,
         pop_total = B01001_001E,
         pop_16_plus = adult_pop,
         pop_65_plus = adults_65_plus,
         pop_65_plus_fraction = senior_frac,
         doses_administered,
         one_dose,
         fully_vaccinated,
         coverage_one_dose_total_pop,
         coverage_one_dose_16_plus_only = coverage,
         infection_rate = attack_rate_mean,
         SVI,
         median_income = median_incomeE,
         EP_LIMENG, EP_MINRTY, F_MINRTY, EP_CROWD, EP_POV, F_POV,
         contains("RPL_THE")
         ) %>%
  mutate(coverage_one_dose_total_pop = one_dose / pop_total) %>%
  glimpse()

vax_o %>% glimpse()

vax_o %>% apply(2, function(x) sum(is.na(x)))

write_csv(vax_o %>%
          as.data.frame() %>%
          select(-geometry),
          "austin_vaccine_map_data.csv")

vax_o %>% filter(ZCTA == "78725")
vax_o %>% filter(ZCTA == "78660")

vax_sub_my$RPL_THEME1 %>% hist()
vax_sub_my$RPL_THEME2 %>% hist()

plot(vax_sub_my$SVI, vax_sub_my$coverage)

summary(lm(vax_o$coverage_one_dose_16_plus_only ~ vax_o$SVI + vax_o$pop_65_plus_fraction))

vax_sub_my <- vax %>%
  filter(COUNTY == "Travis" | PO_NAME == "Austin") %>%
  glimpse()

vax_sub2_my <- vax_sub_my %>%
  left_join(burden) %>%
  left_join(attack,
            by = c("GEOID" = "ZIP")) %>%
  glimpse()

vax_sub2_my %>% filter(GEOID == "78725")
vax_sub2_my %>% filter(GEOID == "78660")
